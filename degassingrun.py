...
class COHS_degassing:
    ...
    def degassing_redox(self, df_results, index, e_balance_initial, sigma):
        ...
        phi_volatiles = Fugacity(self.P, self.T)
        df_results["phi_H2O"][index] = phi_volatiles.phiH2O
        df_results["phi_H2S"][index] = phi_volatiles.phiH2S
        df_results["phi_SO2"][index] = phi_volatiles.phiSO2
        df_results["pressure"][index] = self.P
        ...
        for j in range(1, def_variables.n):
            ...
            cohs = S_fO2(self.P, self.Tk, melt_comp_updated.composition, model_choice=self.S_Fe_choice)
            ...
            if abs(np.log10(fo2_tr[j]) - np.log10(fo2_tr[j - 1])) < sigma:
                df_results["kd_RxnI"][index] = kd1_tr[j]
                df_results["kd_RxnIa"][index] = kd1a_tr[j]
                df_results["kd_RxnII"][index] = kd2_tr[j]
                df_results["SO2/ST"][index] = rs_f_tr[j]
                df_results["kd_combined_molar"][index] = kd_combined_tr[j]
                df_results["kd_combined_wt"][index] = kdS_wt
                df_results["DS_bulk"][index] = DS_wt
                df_results["XS_melt"][index] = XS_m_tr[j]
                df_results["wS_melt"][index] = wS_m_tr[j]
                df_results["wS_fluid"][index] = wS_f_tr[j]
                df_results["XS_fluid"][index] = XH2S_f_tr[j] + XSO2_f_tr[j]
                df_results["XH2S_fluid"][index] = XH2S_f_tr[j]
                df_results["XSO2_fluid"][index] = XSO2_f_tr[j]
                df_results["SO2_fugacity"][index] = XSO2_f_tr[j] * phi_volatiles.phiSO2 * self.P * 10
                df_results["H2S_fugacity"][index] = XH2S_f_tr[j] * phi_volatiles.phiH2S * self.P * 10
                df_results["XH2O_fluid"][index] = XH2O_fluid_tr[j]
                df_results["XCO2_fluid"][index] = XCO2_fluid_tr[j]
                if wH2O_m_tr[j] <= df_results["wH2O_melt"][index - 1]:
                    df_results["wH2O_melt"][index] = wH2O_m_tr[j]
                else:
                    df_results["wH2O_melt"][index] = df_results["wH2O_melt"][index - 1]
                df_results["wCO2_melt"][index] = wCO2_m_tr[j]
                df_results["melt_fraction"][index] = melt_fraction_tr[j]
                df_results["vapor_fraction"][index] = vapor_fraction_tr[j]
                df_results["crystal_fraction"][index] = crystal_fraction_tr[j]
                df_results["water_fugacity"][index] = df_results["XH2O_fluid"][
                                                          index] * self.P * 10 * phi_volatiles.phiH2O
                df_results["fO2"][index] = np.log10(fo2_tr[j])
                df_results["FMQ"][index] = fo2_degassing.fmq()
                df_results["ferric_ratio"][index] = ferric_ratio_tr[j]
                df_results["S6+/ST"][index] = rs_m_tr[j]
                df_results["FeOT"][index] = melt_comp_updated.composition["FeOT"]
        ...
        df_results["sulfate_m"][index] = df_results["melt_fraction"][index] * (df_results["wS_melt"][index] / 10000) * \
                                         df_results["S6+/ST"][index] / 32.065
        df_results["sulfide_m"][index] = df_results["melt_fraction"][index] * (df_results["wS_melt"][index] / 10000) * \
                                         (1 - df_results["S6+/ST"][index]) / 32.065
        df_results["SO2_f"][index] = df_results["vapor_fraction"][index] * df_results["wS_fluid"][index] * \
                                     df_results["SO2/ST"][index] / 32.065
        df_results["H2S_f"][index] = df_results["vapor_fraction"][index] * df_results["wS_fluid"][index] * (
                1 - df_results["SO2/ST"][index]) / 32.065
        df_results["ferric"][index] = melt_comp_updated.composition["FeOT"] * df_results["melt_fraction"][index] * \
                                      df_results["ferric_ratio"][index] / (55.845 + 15.999)
        df_results["ferrous"][index] = melt_comp_updated.composition["FeOT"] * df_results["melt_fraction"][index] * \
                                       (1 - df_results["ferric_ratio"][index]) / (55.845 + 15.999)
        df_results["ferric_cr"][index] = df_results["ferric_cr"][index - 1] + Fe3_cr
        df_results["ferrous_cr"][index] = df_results["ferrous_cr"][index - 1] + Fe2_cr
        df_results["electron_balance"][index] = df_results["sulfide_m"][index] * 8 + df_results["H2S_f"][index] * 8 \
                                                + 2 * df_results["SO2_f"][index] + df_results["ferrous"][index] + \
                                                df_results["ferrous_cr"][index]
        df_results["SCSS"][index] = solubility.SCSS_smythe()
        df_results["SCAS"][index] = solubility.SCAS_Zajacz_Tsay()
        df_results["fH2"][index] = re_new.hydrogen_equilibrium(fo2=10 ** df_results["fO2"][index],
    ...
    def degassing_noredox(self, delta_FMQ, df_results, index):
        ...
        df_results["fO2"][index] = fo2_degassing.fmq() + delta_FMQ
        # Fe3+/FeT is recalculated with the fixed delta_FMQ
        df_results["ferric_ratio"][index] = fo2_degassing.fe_ratio(df_results["fO2"][index])
        ...
        df_results["kd_RxnI"][index] = re.kd_rxn1(xh2o=df_results["XH2O_fluid"][index - 1])
        df_results["kd_RxnII"][index] = re.kd_rxn2(fo2=10 ** (df_results["fO2"][index]))
        df_results["kd_RxnIa"][index] = re.kd_rxn1a(fo2=10 ** df_results["fO2"][index])
        df_results["S6+/ST"][index] = rs_melt.sulfate
        df_results["SO2/ST"][index] = re.gas_quilibrium(fo2=10 ** df_results["fO2"][index],
        ...
        df_results["kd_combined_molar"][index] = (df_results["SO2/ST"][index] * df_results["kd_RxnIa"][index] +
                                                  (1 - df_results["SO2/ST"][index]) * df_results["kd_RxnI"][index]) * (
        ...
        df_results["XS_fluid"][index] = df_results["XS_melt"][index - 1] * df_results["kd_combined_molar"][index]
        df_results["XSO2_fluid"][index] = df_results["XS_fluid"][index] * df_results["SO2/ST"][index]
        df_results["XH2S_fluid"][index] = df_results["XS_fluid"][index] * (1 - df_results["SO2/ST"][index])
        ...
        if self.xlt_choice == 1:  # if crystallization is enabled
            ...
            if root.x[1] <= 0:
                df_results["vapor_fraction"][index] = 0
            else:
                df_results["vapor_fraction"][index] = root.x[1]

            if root.x[6] <= 0:
                df_results["crystal_fraction"][index] = 0
            else:
                df_results["crystal_fraction"][index] = root.x[6]
        ...
        df_results["melt_fraction"][index] = fm
        df_results["XH2O_fluid"][index] = XH2O_f
        df_results["XCO2_fluid"][index] = XCO2_f
        df_results["wH2O_melt"][index] = wtH2O_m
        # if wtH2O_m <= df_results["wH2O_melt"][i-1]:
        #     df_results["wH2O_melt"][i] = wtH2O_m
        # else:
        #     df_results["wH2O_melt"][i] = df_results["wH2O_melt"][i-1]
        df_results["wCO2_melt"][index] = wtCO2_m
        ...
        df_results["kd_combined_wt"][index] = df_results["wS_fluid"][index] * 10000 / df_results["wS_melt"][index - 1]

        if df_results["vapor_fraction"][index] + df_results["crystal_fraction"][index] > 0:
            df_results["DS_bulk"][index] = df_results["kd_combined_wt"][index] * df_results["vapor_fraction"][index] / (
                    df_results["vapor_fraction"][index] + df_results["crystal_fraction"][index])
        else:
            df_results["DS_bulk"][index] = df_results["kd_combined_wt"][index]

        df_results["wS_melt"][index] = self.S_0 / (
                fm * (1 - df_results["DS_bulk"][index]) + df_results["DS_bulk"][index])
        df_results["XS_melt"][index] = (df_results["wS_melt"][index] / (10000 * 32.065)) / \
        ...
        df_results["water_fugacity"][index] = df_results["XH2O_fluid"][index] * df_results["pressure"][index] \
                                              * phi_volatiles.phiH2O * 10
        df_results["SO2_fugacity"][index] = df_results["XSO2_fluid"][index] * phi_volatiles.phiSO2 * \
                                            df_results["pressure"][index] * 10
        df_results["H2S_fugacity"][index] = df_results["XH2S_fluid"][index] * phi_volatiles.phiH2S * \
                                            df_results["pressure"][index] * 10
