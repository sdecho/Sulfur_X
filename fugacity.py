...
class Fugacity:
    ...
    def phi_h2(self):
        ...
        Pcr = 12.9696  # critical pressure in bar
        Pr = self.P / Pcr
        P0r = P0 / Pcr
        Tr = self.T / Tcr
        ...
        C = CQ1 + CQ2 * Tr + CQ3 * (Tr ** -1) + CQ4 * (Tr ** 2) + CQ5 * (Tr ** -2) + CQ6 * (Tr ** 3) + \
            CQ7 * (Tr ** -3) + CQ8 * np.log(Tr)
        D = DQ1 + DQ2 * Tr + DQ3 * (Tr ** -1) + DQ4 * (Tr ** 2) + DQ5 * (Tr ** -2) + DQ6 * (Tr ** 3) + \
            DQ7 * (Tr ** -3) + DQ8 * np.log(Tr)
        # Equation (10) from Shi & Saxena 1992
        Zcomp = A * np.log(Pr / P0r) + B * (Pr - P0r) + (C / 2) * (Pr ** 2 - P0r ** 2)+(D / 3) * (Pr ** 3 - P0r ** 3)
        # Equation (9) from Shi & Saxena 1992
        lnphiH2 = Zcomp - np.log(self.P)
        return np.exp(lnphiH2)
